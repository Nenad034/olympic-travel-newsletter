...
export default function Shell({
  ...
  // Dokovano polje agenta je JEDNO i živi u desnom panelu: `AiChatBox` se montira tačno jednom
  // (na dnu ovog stabla) i FIZIČKI se premešta u slot desnog panela kad se panel otvori. Da se
  // umesto toga renderovao unutar `RightPanel`-a, zatvaranje panela bi ga odmontiralo, pa bi se
  // izgubila istorija razgovora i nedovršen tekst u polju.
  //
  ...
  // Jedina druga pozicija je POSEBAN TAB `/ai-agent` (sopstveni primerak, sopstveni razgovor).
    ...
    <TabsProvider>
      <InspectorProvider value={inspector}>
        {/* Prvo prilaganje konteksta („#") otvara desni panel, jer se tamo agent i nalazi — osim
          kad je već u posebnom tabu, gde je i bez panela pred očima. */}
        <AiContextProvider
          onFirstAdd={() => {
            if (pathname !== '/ai-agent') openRight();
          }}
        >
          <div className="flex h-screen flex-col overflow-hidden bg-bg text-ink">
            ...
            <div className="flex min-h-0 flex-1">
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                ...
                — jedno mesto umesto ožičenja svakog ekrana ponaosob. Agent NIJE potomak ovog
                elementa: inače bi čitao sopstvenu istoriju razgovora. */}
              ...
              {rightOpen && (
                <>
                  ...
                  >
                    ...
                    <RightPanel onClose={toggleRight} aiSlotRef={setAiSlot} />
                  </div>
                </>
              )}
          ...
          {/* JEDINI DOKOVANI `AiChatBox`, u stabilnom domaćinu koji se premešta u slot desnog panela
          (poseban tab `/ai-agent` montira svoj primerak, sa sopstvenom istorijom).
          Dok je desni panel zatvoren, domaćin stoji ovde parkiran: omotač je `fixed` i nulte
